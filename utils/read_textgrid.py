...
def read_one_textgrid(textgrid_file="", phoneme_list=[]) -> list[float]:
    ...
    textgrid_results = TextGrid.read(textgrid_file)

    interval_phonemes = textgrid_results[1]

    for i, interval in enumerate(interval_phonemes):
        phoneme = interval[i].mark
        start_time = interval[i].minTime
        end_time = interval[i].maxTime
